...
class PsychAdapter(nn.Module):
    ...
    def __init__(self, model_name_or_path, latent_size):
        ...
        # head_dim = hidden_size / num_attention_heads (usually 128 for Llama 3)
        if not hasattr(self.model_config, "head_dim"):
            self.model_config.head_dim = self.model_config.hidden_size // self.model_config.num_attention_heads
        
        # Ensure num_key_value_heads exists (defaults to num_attention_heads if not in config)
        if not hasattr(self.model_config, "num_key_value_heads") or self.model_config.num_key_value_heads is None:
    ...
    def initialize_model(self, args):
        ...
        # Ensure the tokenizer has a pad_token (Essential for Llama/Gemma)
        ...
        # Handle cases where BOS might be missing (e.g., some GPT-2 configs)
    ...
    def inference(self, prompting_text = None, sentence_embedding = None, args = None, device = None):
        ...
        if prompting_text is None:
            ...
            decoder_input_ids = torch.tensor([[self.bos_token_id] for _ in range(batch_size)], device=device).long()
        else:
            ...
            prompting_text_encoded = self.tokenizer.encode(
                prompting_text.strip(), 
                add_special_tokens=True # Automatically adds <bos> if configured
            )

            # 2. Create the tensor and use .repeat() to handle batch_size
            decoder_input_ids = torch.tensor(prompting_text_encoded, device=device).long()
            decoder_input_ids = decoder_input_ids.unsqueeze(0).repeat(batch_size, 1)
        past = transformed_embeddings
